                ...
                { props.isCurrentPageNav === 1 ?
                 ...
                 <div onClick={() => {props.SetCurrentTheme(props.Themes[5])}} className={s.TORThemeWrap}><NavLink className={s.TORTheme}>{props.Themes[5].name}</NavLink></div>
                   ...
                   <div className={s.CurrentContent}>
                       {props.CurrentSubtopics =! "" ?
                       <div className={s.CurrentContentContentInfo}>{props.CurrentSubtopics.map(el => {
                        return <div> <CurrentSubtopic Text={el.parText} Title={el.Title}/></div>
                        })}</div> :
                         <div></div>}
                       <div className={s.CurrentContentContent}>{props.CurrentTheme.subtopics.map(el => {
                        return <div className={s.SubTopic}><img className={s.backgroundImg} src={el.backgroundImg}/><div className={s.SubTopicName} onClick={() => {debugger; props.SetCurrentSubtopics(el.subtopicText)}}>{el.topicName}</div></div>
                       })}</div>
                   </div>
                     </div>
                :
