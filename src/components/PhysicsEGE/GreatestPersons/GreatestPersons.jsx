    ...
    return (
        <div>
            ...
            <div className={s.ShortInfo}>
                ...
                <div>{props.CurrentChoosenScientist.MainInfo}</div>
                <div className={s.VkladTitle}>Вклад в науку:</div>
                <ul>
                    {props.CurrentChoosenScientist.Discoveries.map(el => {
                        return (
                            <li className={s.discovery}><NavLink className={el.linK != '' ? s.TrueLink : s.LimeLink} to={el.linK}>{el.title}</NavLink></li>
                        )})}
                </ul>
            </div>
            </div>
